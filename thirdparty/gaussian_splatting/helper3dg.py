...
def getcolmapsinglecustom(folder, offset):
    """
    Full COLMAP SfM pipeline for datasets without known camera poses/intrinsics.
    Uses colmap mapper (estimates poses + intrinsics) instead of point_triangulator.
    """
    folder = os.path.join(folder, "colmap_" + str(offset))
    assert os.path.exists(folder)
    ...
    # mapper estimates camera poses and intrinsics from scratch; outputs to distorted/sparse/0/
    mapper = "colmap mapper --database_path " + dbfile + " --image_path " + inputimagefolder + " --output_path " + distortedmodel
    exit_code = os.system(mapper)
    if exit_code != 0:
        exit(exit_code)

    # mapper writes to distorted/sparse/0/, so point undistorter there
    img_undist_cmd = "colmap image_undistorter --image_path " + inputimagefolder \
        + " --input_path " + os.path.join(distortedmodel, "0") \
        + " --output_path " + folder \
        + " --output_type COLMAP"
    exit_code = os.system(img_undist_cmd)
    if exit_code != 0:
